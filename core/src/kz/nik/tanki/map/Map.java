...
public class Map {
    public enum WallType {
        ...
        boolean bulletPossible;

        boolean destructible;

        WallType(int index, int maxHp, boolean destructible,boolean bulletPossible,boolean unitPossible) {
            this.index = index;
            this.maxHp = maxHp;
            this.destructible = destructible;
            this.unitPossible = unitPossible;
            this.bulletPossible =bulletPossible;
        }
    }
    ...
    public Map(TextureAtlas atlas) {
        ...
        this.mapTexture = atlas.findRegion("block2");
        ...
        /*for (int i = 0; i < SIZE_X; i++) {
            ...
            cells[i][1].changeType(WallType.INDESTRUCTABLE);
            cells[i][SIZE_Y - 2].changeType(WallType.INDESTRUCTABLE);
        }
        for (int i = 0; i < SIZE_Y; i++) {
            cells[0][i].changeType(WallType.INDESTRUCTABLE);
            cells[SIZE_X - 1][i].changeType(WallType.INDESTRUCTABLE);
            cells[1][i].changeType(WallType.INDESTRUCTABLE);
            cells[SIZE_X - 2][i].changeType(WallType.INDESTRUCTABLE);
        }*/
    }
    ...
    public void checkWallAndBulletsCollision(Bullet bullet) {
        ...
        if (cx >= 0 && cy >= 0 && cx <= SIZE_X && cy <= SIZE_Y) {
            if (!cells[cx][cy].type.bulletPossible) {
                cells[cx][cy].damage();
                bullet.deactivate();
            }
        }
    ...
    public void render(SpriteBatch batch) {
        for (int i = 0; i < 1280 / 40; i++) {
            for (int j = 0; j < 720 / 40; j++) {
                batch.draw(mapTexture, i * 40, j * 40);
            }
        }
        for (int i = 0; i < SIZE_X; i++) {
            for (int j = 0; j < SIZE_Y; j++) {
